...
from dashboard.models import KPI, Submission
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        if kpi_id:
            try:
                ...
                messages.error(request, "Siz tanlagan KPI da xatolik yuz berdi. Iltimos yana bir bor ununib ko'ring.")
                current_kpi = KPI.objects.order_by("-end_date").first()
        else:
            current_kpi = KPI.objects.order_by("-end_date").first()
            
        if user.role == User.ADMIN:
            ctx = {
                "users_count": User.objects.exclude(role=User.ADMIN).count(),
                "department_count": Department.objects.all(),
            }
            if current_kpi:
                ctx["other_kpis"] = KPI.objects.exclude(id=current_kpi.id),
                ctx["current_kpi"] = current_kpi,
                ctx["submissions_count"] = current_kpi.submissions.count(),
                ctx["clauses_count"] = current_kpi.clauses.count(),
                ctx["criterions_count"] = current_kpi.criterions.count(),
